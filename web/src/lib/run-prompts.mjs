 ...
 * message, every time, so both ran at the head of every single run: measured at
 * 3 shell-outs and a network round trip before any real work.
 *
 ...
 * headless, not merely redundant. The setup check is mostly redundant: the
 * route already refuses an evaluate without cv.md.
 *
 * What IS lost is doctor's `unpersonalized` warning, which catches a
 * modes/_profile.md still carrying template content — the failure that once
 * scored a whole batch against a stranger's targeting. That belongs on the
 * Config page, once, rather than at the head of every run; not wired yet.
 *
 * Prepended to every kind, and only in the web layer. AGENTS.md is untouched,
 * so interactive CLI sessions keep the full cold-start check.
 */
