...
import { buildTitleFilter, buildTitleFilterOverrides, buildTitleFilterWithOverrides, buildLocationFilter, buildContentFilter, matchedTitleKeywords, recordTitleReject, flushTitleRejects, loadSeenUrls, normalizeUrlForDedup, appendToPipeline, appendToScanHistory, loadBlacklist, parseSinceDays, PORTALS_PATH, PIPELINE_PATH } from './scan.mjs';
...
async function main() {
  ...
  const processJobs = async (jobs, sourceName, provider, companySlug) => {
    for (const job of jobs) {
      ...
      if (!titleFilter(job.title, companySlug)) { recordTitleReject(job.title, job.company, job.source || ats); continue; }
      // job.url is passed so the location filter can fall back to the URL's own
      // location segment when the provider reports a rolled-up "N Locations" string;
      // job.title so a title-stated remote role survives a city-only location.
